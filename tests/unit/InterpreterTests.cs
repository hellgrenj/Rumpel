...
namespace unit
{
    public class InterpreterTests
    {
        ...
        public void InferSchemaAndValidate_returns_true_and_empty_errors_if_json_ok()
        {
            ...
            var trans = new Transaction() { Request = new() { Method = "POST", Path = "/Unit/tests" }, Response = new() { RawBody = expectedJsonString, StatusCode = 200 } };
            var (isValid, errorMessages) = Interpreter.InferSchemaAndValidate(jsonString, trans, 200, new List<string>());
        ...
        [Fact]
        public void InferSchemaAndValidate_returns_false_and_errorMessages_if_wrong_status_code()
        {
            var jsonString = @"
            { 
                ""id"": 1, 
            ...
            {
                Request = new() { Method = "POST", Path = "/Unit/tests" },
                Response = new()
                {
                    RawBody = expectedJsonString,
                    StatusCode = 200
                }
            };
            var (isValid, errorMessages) = Interpreter.InferSchemaAndValidate(jsonString, trans, 201, new List<string>());

            Assert.False(isValid);
            Assert.Contains("expected status code 200", errorMessages[0]);
        }

        [Fact]
        public void InferSchemaAndValidate_returns_true_if_wrong_status_code_but_ignore_flag()
        {
            var jsonString = @"
            { 
                ""id"": 1, 
            ...
            var (isValid, errorMessages) = Interpreter.InferSchemaAndValidate(jsonString, trans, 201, new List<string>() { "--ignore-assert-status-code" });
        ...
        public void InferSchemaAndValidate_returns_false_and_errorMessages_if_wrong_property_type()
        {
            ...
            var trans = new Transaction()
            {
                Request = new() { Method = "POST", Path = "/test" },
                Response = new()
                {
                    RawBody = expectedJsonString,
                    StatusCode = 200
        ...
        public void InferSchemaAndValidate_returns_true_if_unexpected_array_length_but_ignore_flag()
        {
            ...
            var (isValid, errorMessages) = Interpreter.InferSchemaAndValidate(jsonString, trans, 200, new List<string>() { "--ignore-assert-array-length" });
